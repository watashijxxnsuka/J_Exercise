...
public class _13_ThreadPool01 {
	public static void main(String[] args) {
		...
		// 2. SumThread01, SumThread02 를 스레드풀 대기 큐에 등록
		threadPool.execute(new SumThread01());
		threadPool.execute(new SumThread02());
//		threadPool.execute(new SumThread01());
		
		// 3. shutDownNow(): InterruptedException 을 발생시켜서
		// 스레드풀의 모든 스레드를 종료시킨다.
//		threadPool.shutdownNow();
		
		// 4. shutdown(): 메인스레드가 종료되도, 대기 큐에 있는 모든 스레드들의
		// 작업을 모두 마친 후에 스레드풀을 종료하는 메소드
		threadPool.shutdown();
