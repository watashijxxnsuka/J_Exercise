...
// 그냥 변수를 선언해도 컴파일러에 의해서 public static final 이 자동으로 붙어서 생성된다.
// 메소드도 마찬가지로 그냥 선언해도 public abstract가 붙어서 생성된다.

// 인터페이스의 메소드는 추상메소드이기 때문에 구현부({}블록)를 만들 수 없다.

// 인터페이스도 추상클래스의 일종이기 때문에 객체를 만들 수 없어서 자식객체를 사용한다.

// 변수를 선언할 때 타입으로는 지정가능

// 인터페이스의 변수는 public final이 붙고, 메소드에는 public abstract가 붙기 때문에,
...
//	}
	...
	// 인터페이스에 기능을 추가하면 상속받은 모든 자식클래스를 수정해야하는 문제 발생.
